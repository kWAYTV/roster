...
export function onAccountStatus(
  handler: (status: StatusPayload) => void
): Promise<UnlistenFn> {
  return listen<StatusPayload>("account-status", (event) =>
...
export function onImportRequest(
...
export function onCooldownFinished(
  handler: (names: string[]) => void
): Promise<UnlistenFn> {
  return listen<string[]>("cooldown-finished", (event) =>
    handler(event.payload)
  );
}
